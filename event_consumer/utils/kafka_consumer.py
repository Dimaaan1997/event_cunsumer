...
class KafkaConsumerCluster:
    ...
    def __init__(self, kafka_url: str, consumers_configs: list[KafkaConsumerConfig]):
    ...
    async def run_cluster(self):
        for config in self.consumers_configs:
            ...
            self.consumer_tasks.append(asyncio.create_task(self._run_consumer_batch(consumer=consumer, config=config)))
    ...
    async def _run_consumer(self, consumer: AIOKafkaConsumer, config: KafkaConsumerConfig):
        await consumer.start()
        await log.ainfo(event="consumer started", topic=config.topic, group=config.consumer_group)
    ...
    async def _run_consumer_batch(self,  consumer: AIOKafkaConsumer, config: KafkaConsumerConfig):
        await consumer.start()
        await log.ainfo(event="consumer started", topic=config.topic, group=config.consumer_group)
        buffer = []
        while True:
            ...
            # print(f"Вставлено {len(batch)} сообщений")
            # await consumer.commit()

            if len(buffer) >= BATCH_SIZE:
                insert_batch = buffer[:BATCH_SIZE]
                del buffer[:BATCH_SIZE]
                await config.worker(insert_batch)
                print(f"Вставлено {len(insert_batch)} сообщений")
                await consumer.commit()
